...
namespace MathLib
{
	...
	{
		typename T::value_type tmp = RecipSqrt(LengthSq(rhs));
		return rhs * tmp;
	}
    ...
    float3 ComputeBaryCentric2D(const float3 &Point, const float3 &A, const float3 &B, const float3 &C)
    {
        ...
        const float3 TriNorm = Cross((B-A), (C-A));

        // Check the size of the triangle is reasonable (TriNorm.Size() will be twice the triangle area)
        if(LengthSq(TriNorm) <= SMALL_NUMBER)
        {
            return float3(-1,1,1);
        }

        const float3 N = Normalize(TriNorm);

        // Compute twice area of triangle ABC
        const float AreaABCInv = 1.0f / Dot(N, TriNorm);

        // Compute a contribution
        const float AreaPBC = Dot(N, Cross((B-Point), (C-Point)));
        const float a = AreaPBC * AreaABCInv;

        // Compute b contribution
        const float AreaPCA = Dot(N, Cross((C-Point), (A-Point)));
        const float b = AreaPCA * AreaABCInv;

        // Compute c contribution
        return float3(a, b, 1.0f - a - b);
    }
    ...
    // vPostion是观察点位置向量，vLookAt是观察方向向量，vUp向上向量
    template  float4x4 LookAtRH(const float3& vEye, const float3& vAt, const float3& vUp) noexcept;
    template<typename T>
        Matrix4_T<T>
    LookAtRH(const Vector_T<T, 3> &vEye, const Vector_T<T, 3> &vAt, const Vector_T<T, 3> &vUp) noexcept
    {
    ...
    // glOrtho
    template float4x4 OrthoOffCenterRH(float left,float right, float bottom, float top, float near, float far) noexcept;
    template<typename T>
    ...
    template  float4x4 PerspectiveOffCenterRH(float left,float right, float bottom, float top, float near, float far) noexcept;
    template<typename T>
    Matrix4_T<T> PerspectiveOffCenterRH(T left,T right, T bottom, T top, T near, T far) noexcept
    {
        ...
        return Matrix4_T<T>(
                near2 / invWidth,	0,				        (right + left) * invWidth,			0,
                0,					near2 / invHeight,		(top + bottom) * invHeight,			0,
    ...
    template float4 MatrixMulVector(const float4x4& mat, const float4& vec) noexcept;
    template<typename T>
        Vector_T<T, 4>
            MatrixMulVector(const Matrix4_T<T> &mat, const Vector_T<T, 4> &vec) noexcept
    {
        return Vector_T<T, 4>(vec.x() * mat(0, 0) + vec.y() * mat(0, 1) + vec.z() * mat(0, 2) + vec.w() * mat(0, 3),
